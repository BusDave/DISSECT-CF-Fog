...
/**
 * Single-workflow training data collection for the HEFT-DS scheduler.
 * Mirror of {@link HeftLoggingDemo} but using {@link HeftDsSchedulerWithLogging}.
 */
public class HeftDsLoggingDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ArrayList<HeftDsSchedulerWithLogging> loggers = new ArrayList<>();
        for (int i = 0; i < clusters.size(); i++) {
            Pair<String, ArrayList<WorkflowJob>> jobs =
                    WorkflowJobModel.loadWorkflowXml(workflowFile, Integer.toString(i));
            HeftDsSchedulerWithLogging sch =
                    new HeftDsSchedulerWithLogging(clusters.get(i), instance, null, jobs);
