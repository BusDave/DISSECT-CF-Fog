...
public class IoTWorkflowFourRegressionSimulation {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var jobs = WorkflowJobModel.loadWorkflowXml(workflowFile, "");

        ArrayList<WorkflowComputingAppliance> computeNodes = new ArrayList<>(workflowArchitecture.keySet());
        Instance instance = workflowArchitecture.values().iterator().next();

        // Initialize EnergyDataCollector for each computing appliance
        int nodeIndex = 1;
        for (WorkflowComputingAppliance node : computeNodes) {
            new EnergyDataCollector("node-" + nodeIndex, node.iaas, true);
            nodeIndex++;
        }

        WorkflowExecutor.getIstance().submitJobs(new MaxMinScheduler(computeNodes, instance, actuatorArchitecture, jobs));
    ...
    private static ArrayList<Actuator> getActuatorArchitecture() {
        ArrayList<Actuator> actuatorArchitecture = new ArrayList<>();
